...
# yum install rrdtool perl-rrdtool rrdtool-devel apr-devel \
              libconfuse libconfuse-devel expat \
              expat-devel pcre pcre-devel
...
\begin{verbatim}
# yum install apr-devel libconfuse libconfuse-devel expat \
              expat-devel pcre pcre-devel
...
edguser  24951  1  0 19:47 /usr/sbin/slapd -f \
                           /opt/bdii/etc/bdii-slapd.conf \
                           -h ldap://osweb.teipir.gr:2170 \
                           -u edguser
edguser  24990  1  0 19:47 /usr/bin/python \
...
[root@osweb ~]# cat \
/opt/glite/etc/gip/provider/glite-provider-ganglia-wrapper
#!/bin/bash
...
Finally the following example may return only the $ProcessorLoad$ node of the $Host$ that has the attribute Name set to $xenia.teipir.gr$:
\begin{verbatim}
//glue:Host[@glue:Name='xenia.teipir.gr']/glue:ProcessorLoad
\end{verbatim}
